...
f_candidate_dict_name = 'candidate_dict_{}.txt'.format('DL' if DL == True else 'L')
names = []
cuisines = []
locations = []
numbers = []
...
def load_entity_dict():
    ...
    api = ['api_call']
    f.close()
    return names, cuisines, locations, numbers, prices, phones, post_codes, api


def extract_entity(word):
    names, cuisines, locations, numbers, prices, phones, post_codes, api = load_entity_dict()
    if word in names:
        return '<name>'
...
def build_candidate_dict(src_file, DL, dst_root=db_root):
    candidate_dict = {}
    if DL == True:
        with open(src_file) as src:
            lines = src.readlines()
        ...
        f = open(dst_root + f_candidate_dict_name, 'w')
...
def get_candidate_dict(DL=True):
    ...
    try:
        ...
        return build_candidate_dict(data_root + f_raw_candidates_name, DL)
